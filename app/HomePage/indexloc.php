...
<body>
	...
	<div class="all">
		...
		echo shell_exec("../exec.sh root root whoami");
		shell_exec("../exec.sh " . $_SESSION['user'] . " " . $_SESSION['pass'] . " 'docker node ls 2>&1 | grep Worker'");

		if (shell_exec("../exec.sh " . $_SESSION['user'] . " " . $_SESSION['pass'] . " 'docker node ls 2>&1 | grep Worker'") != "") {
			echo '
					<table>
						<tr>
							<td>
		...
		} else if (shell_exec("../exec.sh " . $_SESSION['user'] . " " . $_SESSION['pass'] . " 'docker node ls 2>&1 | grep Use'") != "") {
		...
		';
		} else if ((trim(shell_exec("../exec.sh " . $_SESSION['user'] . " " . $_SESSION['pass'] . " \"docker node ls | grep '*' |awk '{print $2}'\"")) == "*")) {
			echo '
		...
		if (isset($_POST['submitx4'])) {
			...
			echo shell_exec("../exec.sh " . $_SESSION['user'] . " " . $_SESSION['pass'] . " 'docker node ls 2>&1'");
			echo "</h3>";
			echo '
		...
		if (isset($_POST['submitx5'])) {
			echo shell_exec("../exec.sh " . $_SESSION['user'] . " " . $_SESSION['pass'] . " 'docker swarm leave -f'");
