...
function check_url($url) {
   ...
   return (bool)preg_match('#^HTTP/.*\s+[(200|301|302)]+\s#i', $headers);
				...
				if (!( shell_exec("echo ".$_SESSION['pass']." | sudo -S -u ".$_SESSION['user']. " docker node ls 2>&1 | grep Use")!=""))
				{	
					header( 'Location: ../LoginPage/loginh.php' );	
				}
?>
			...
			<div class="form-group">
				<form  name="form1" method="post">
					<div class="table-responsive">
						...
						<tr><th><center>choose an IP address to advertise </center></th></tr>
						<tr><td><center><input type="text" name="adv" required/></center></tr>
...
/*---------------------------------------------------------------------------------------------LEADER----------------------------------------------------------------------------------------------------*/
	...
	if(isset($_POST['adv']))
	{
		...
		$jtwork = "docker swarm join --token " . rtrim(shell_exec("echo " . $pass . " | sudo -S -u " . $user . "  docker swarm join-token worker -q")) . " " . $_POST["adv"] . ":2377"; #maybe add a field to modify port
		$jtman = "docker swarm join --token " . rtrim(shell_exec("echo " . $pass . " | sudo -S -u " . $user . "  docker swarm join-token manager -q")) . " " . $_POST["adv"] . ":2377";  
	...
	/*---------------------------------------------------------------------------MANAGERS---DOCKER---MACHINES--------------------------------------------------------------------*/	
		...
		if(isset($_POST['dmmu'], $_POST['dmmip'], $_POST['dmmp'], $_POST['dmm']))
		{
			for($n1=0, $count1 = count($_POST['dmmu']);$n1<$count1;$n1++) 
			{
			 	...
			 	echo shell_exec("echo  " . $pass . " | sudo -S -u " . $user . " sshpass -p " . $dmmanp . " ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no " . $dmmanu . "@" . $dmmanip ."   \" echo   ".$dmmanp." | sudo -S docker-machine ssh  ".$dockm." ".$jtman."\" 2>&1 ");	
