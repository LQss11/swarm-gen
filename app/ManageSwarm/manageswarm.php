...
function check_url($url) {
   ...
   return (bool)preg_match('#^HTTP/.*\s+[(200|301|302)]+\s#i', $headers);
				...
				if (!(trim(shell_exec("echo ".$_SESSION['pass']." | sudo -S -u ".$_SESSION['user']. " docker node ls | grep '*' |awk '{print $2}'"))=="*"))
				{	
					header( 'Location: ../LoginPage/loginh.php' );	
				}
?>
...
<head>
  ...
  <link rel="stylesheet" href="./manageswarm.css">
  
		<link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
		<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.2.0/jquery.min.js"></script>
  <title>Manage swarm</title>
</head>
<style>
input[type][disabled]
{
...
<style>
		...
		.table th {
		  ...
		  padding-bottom: 12px;
		  text-align: left;
		  background-color:#28546C /*#4CAF50*/;
		  color: white;
		}
	</style>
	<style>
			...
			<nav id="nav">
				<?php 	
					if (check_url("http://localhost:3000"))
   					echo '<a href="http://localhost:3000">Monitor</a>';
					if (check_url("http://localhost:9000"))
   					echo '<a href="http://localhost:9000">Manage</a>';
					if (check_url("http://localhost/app/kibana"))
   					echo '<a href="http://localhost/app/kibana">Logs</a>';
				?>
...
echo '<H1 style="margin-top:100px; text-align:center; color:black;"> This Is My Swarm</H1>';

		exec(" echo " . $pass . " | sudo -S -u " . $user . " sudo -S docker node ls -q | xargs docker node inspect   -f '{{ .Description.Hostname }}' &", $hn);
		exec(" echo " . $pass . " | sudo -S -u " . $user . " sudo -S docker node ls -q | xargs docker node inspect   -f '{{ .Status.Addr }}' &", $haddr);
		exec(" echo " . $pass . " | sudo -S -u " . $user . " sudo -S docker node ls -q | xargs docker node inspect   -f '{{ .ManagerStatus.Reachability }}' &", $reachability);
		exec(" echo " . $pass . " | sudo -S -u " . $user . " sudo -S docker node ls -q | xargs docker node inspect   -f '{{ .ManagerStatus.Leader }}' &", $leader);
		exec(" echo " . $pass . " | sudo -S -u " . $user . " sudo -S docker node ls -q | xargs docker node inspect   -f '{{ .Status.State }}' &", $Status);
		exec(" echo " . $pass . " | sudo -S -u " . $user . " sudo -S docker node ls -q | xargs docker node inspect   -f '{{ .Spec.Availability }}' &", $Availability);
#foreach ($output as $line) {
echo '<form method="post">
	<table class="table table-bordered">';	
echo '
		<tr>
			<th style="text-align:center; font-size:25px;">HOSTNAME</th>
			<th style="text-align:center; font-size:25px;">HOST ADDRESS</th>
			<th style="text-align:center; font-size:25px;">Status</th>
			<th style="text-align:center; font-size:25px;">Availability</th>
			<th style="text-align:center; font-size:25px;">MANAGER STATUS</th>
			<!--th style="text-align:center; font-size:25px;">Promote</th-->
		</tr>';
		for($n=0, $count = count($hn);$n<$count;$n++) { 
		   	echo  
		'<tr>';
			echo  "
			<td style='text-align:center; color:black; font-size:20px;'>"."$hn[$n]"."</td>";
			echo  "
			<td style='text-align:center; color:black;'>"."$haddr[$n]"."</td>";
			echo  "
			<td style='text-align:center; color:black;'>"."$Status[$n]"."</td>";
			echo  "
			<td style='text-align:center; color:black;'>"."$Availability[$n]"."</td>";
		if (isset($reachability[$n]) && ($reachability[$n] =="reachable"))
		{
			if (isset($leader[$n]) && ($leader[$n] =="true"))
			$reachability[$n]="Leader";
			else if (isset($leader[$n]) && ($leader[$n] =="false"))
			$reachability[$n]="Manager";
		}
		else
		$reachability[$n]="Worker";
			echo  "
			<td style='text-align:center; color:black;'>"."$reachability[$n]"."</td>";
			/*echo  "
			<td style='text-align:center; color:black;'><input type='submit' name='$hn[$n]' value='PROMOTE'></td>";
$x=$hn[$n];
			if(isset($_POST['$x'])
				if (isset($reachability[$n]) && (!($reachability[$n] =="reachable")))
				{
				shell_exec("echo azerty123 | sudo -S -u lqss sudo -S docker node promote". $hn[$n]);	
				}
...
echo 
	'</table></form>';
		...
		echo "<br>";
		echo '<pre style="font-size:20px; color:black;">';
			echo shell_exec("echo  " . $_SESSION['pass'] . " | sudo -S -u  " . $_SESSION['user'] . " docker node ls ");
		echo '</pre-->';
...
?>
	...
	<center>
		...
		/*echo '
		<a href="http://locahost/app/kibana" >
			<p>LOGGING</p>
			<img src="elastic.png" width="100" height="100">
		</a>
		<a href="http://localhost:9000" >
			<p>MANAGING</p>
			<img  src="port.png" width="100" height="100">
		</a>
		<a href="locahost:3000" >
			<p>MONITORING</p>
			<img src="grafana.png" width="100" height="100" >
		</a>';*/
		?>
	</center>
		...
		<div class="container">
			...
			<h2 align="center">ADD MORE MACHINES TO THE SWARM</h2><br />
			<div class="form-group">
				<form  method="post">
					<div class="table-responsive">

	<hr>
	<center><h3>MANAGER NODES</h3></center>
	<hr>
						<table class="table table-bordered">
						...
						<table class="table table-bordered" id="add_man">
							<tr><th>SSH Username</th><th>SSH IP</th><th>SSH Password</th><th>Docker-Machine Name</th><th>Remove</th></tr>
						</table>
	<hr>
	<center><h3>WORKER NODES</h3></center>
	<hr>
						<table class="table table-bordered">
	...
	<hr><br>
						...
						echo'
							...
							<tr>';
								if (!check_url("http://localhost:9000"))
  						echo'<th  colspan="3"><center>Swarm management</center></th>';
								if (!check_url("http://localhost:3000"))
  						echo'<th>Monitoring</th>';
								if (!check_url("http://localhost/app/kibana"))
  						echo'<th>Logging</th></tr>';
							echo '
							<tr>';if (!check_url("http://localhost:9000"))
  						echo'
						...
						?>	
						<center><input  type="submit" name="submit" formaction="./manageswarm.php" id="submit"  value="Submit" /></center>
						<!--center><input  type="button" name="submit" formaction="../terminal.php"  id="submit"  value="terminal" /></center-->
					</div>
				</form>
...
/*---------------------------------------------------------------------------------------------LEADER----------------------------------------------------------------------------------------------------*/
	...
	#if(isset($_POST['adv']))
	#{
		#shell_exec("echo ".$pass." | sudo -S -u ".$user. " sshpass -p  " . $pass . "  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no  " . $user . "@127.0.0.1 \" docker swarm init --advertise-addr " . $_POST["adv"]."\""); # create a swarm with the adv addr
		
		#echo " Swarm Updated Successfully <br>";
		$jtwork = rtrim(shell_exec("echo " . $pass . " | sudo -S -u " . $user . "  docker swarm join-token worker | grep 2377"));
		$jtman = rtrim(shell_exec("echo " . $pass . " | sudo -S -u " . $user . "  docker swarm join-token manager | grep 2377"));  
			...
			for($n=0, $count = count($_POST['mu']);$n<$count;$n++) 
			{
				...
				$manip  = $_POST['mip'][$n];
		 		$manp  = $_POST['mp'][$n];
				  ...
				  shell_exec("echo ".$pass." | sudo -S -u ".$user. " sshpass -p  " . $manp . "  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no  " . $manu . "@". $manip." \" echo ". $manp ." | sudo -S " . $jtman."\" ");
				...
				if(isset($_POST['ELK']))
				{
					shell_exec("echo ".$pass." | sudo -S -u ".$user. " sshpass -p  " . $manp . "  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no  " . $manu . "@". $manip." \" echo ". $manp ." | sudo -S sysctl -w vm.max_map_count=262144\" ");	
				}		
			}
		}
	/*---------------------------------------------------------------------------MANAGERS---DOCKER---MACHINES--------------------------------------------------------------------*/	
		...
		if(isset($_POST['dmmu'], $_POST['dmmip'], $_POST['dmmp'], $_POST['dmm']))
		{
			for($n1=0, $count1 = count($_POST['dmmu']);$n1<$count1;$n1++) 
			{
			 	...
			 	echo shell_exec("echo  " . $pass . " | sudo -S -u " . $user . " sshpass -p " . $dmmanp . " ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no " . $dmmanu . "@" . $dmmanip ."   \" echo   ".$dmmanp." | sudo -S docker-machine ssh  ".$dockm." ".$jtman."\" ");	
				...
				if(isset($_POST['ELK']))
				{
					shell_exec("echo  " . $pass . " | sudo -S -u " . $user . " sshpass -p " . $dmmanp . " ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no " . $dmmanu . "@" . $dmmanip ."   \" echo   ".$dmmanp." | sudo -S docker-machine ssh  ".$dockm." 'sudo  sysctl -w vm.max_map_count=262144' \" ");	
				}		
			}
		}
		echo "<br>"; 	
	...
	/*----------------------------------------------------------------------------------WORKERS---MACHINES--------------------------------------------------------------------------*/
		if(isset($_POST['wu'], $_POST['wip'], $_POST['wp']))
		{
			for($m2=0, $count2 = count($_POST['wu']);$m2<$count2;$m2++) 
			{
				...
				shell_exec("echo ".$pass." | sudo -S -u ".$user. " sshpass -p  " . $workp . "  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no  " . $worku . "@". $workip." \" echo ". $workp ." | sudo -S " . $jtwork."\"");
				...
				if(isset($_POST['ELK']))
				{
					shell_exec("echo ".$pass." | sudo -S -u ".$user. " sshpass -p  " . $workp . "  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no  " . $worku . "@". $workip." \" echo ". $workp ." | sudo -S sysctl -w vm.max_map_count=262144\"");	
				}	
			}
		}
	...
	/*-----------------------------------------------------------------------------WORKERS---DOCKER---MACHINES--------------------------------------------------------------------------*/	
		if(isset($_POST['dmwu'], $_POST['dmwip'], $_POST['dmwp'], $_POST['dmw']))
		{
			for($m3=0, $count3 = count($_POST['dmwu']);$m3<$count3;$m3++) 
			{
				...
				shell_exec("echo  " . $pass . " | sudo -S -u " . $user . " sshpass -p " . $dmworkp . " ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no " . $dmworku . "@" . $dmworkip ."   \" echo   ".$dmworkp." | sudo -S docker-machine ssh  ".$dockw." ".$jtwork."\" ");	
				...
				if(isset($_POST['ELK']))
				{
					shell_exec("echo  " . $pass . " | sudo -S -u " . $user . " sshpass -p " . $dmworkp . " ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no " . $dmworku . "@" . $dmworkip ."   \" echo   ".$dmworkp." | sudo -S docker-machine ssh  ".$dockw." 'sudo sysctl -w vm.max_map_count=262144' \" ");
				}	
...
if(isset($_POST['ELK']))
{
	...
	shell_exec("echo " . $pass . "  | sudo -S sysctl -w vm.max_map_count=262144");

shell_exec("echo " . $pass . " | sudo -S -u " . $user . " echo \"version: '3'
...
shell_exec('echo ' . $pass . ' | sudo -S -u ' . $user . ' echo \'input {
  udp {
    port  => 5000
    codec => json
  }
...
$cl='echo ' . $pass . ' | sudo -S -u ' . $user . ' docker stack deploy -c logstack.yml elk ';
	addFunction($cl);
...
if(isset($_POST['Portainer']) && ($_POST['Portainer']=="PINSTANCE"))
{
		...
		else{$LID=trim(shell_exec('echo ' . $pass . ' | sudo -S -u ' . $user . ' docker node ls | grep Leader|  awk \'{print $1}\''));}

shell_exec("echo " . $pass . " | sudo -S -u " . $user . " echo \"version: '3.2'
...
volumes:
 ...
 \" > portagent.yml");
	...
	$cl='echo ' . $pass . ' | sudo -S -u ' . $user . ' docker stack deploy -c portagent.yml portainer ';
		addFunction($cl);
      /*-----setup---portainer---pass-----*/	

$ppass=$_POST["PP"]; $padmin=$_POST["PA"]; 
file_put_contents('script.sh', '#!/bin/bash
while true
do
	if [ $(echo ' . $pass . ' | sudo -S -u ' . $user . '  docker service ls | grep portainer/portainer | grep 1/1)!="" ]
	then
		sleep 30
		continue
	 else 
		echo "' . $pass . '" | sudo -S -u ' . $user . ' sshpass -p azerty123 ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ' . $user . '@localhost echo \'' . $pass . '\' | sudo -S curl --include --request POST http://localhost:9000/api/users/admin/init --data \'{"Username":"' . $padmin . '","Password":"' . $ppass . '"}\'
		break
	fi
done');

shell_exec('echo ' . $pass . ' | sudo -S -u ' . $user . ' sudo -S chmod +777 script.sh');
echo shell_exec('echo ' . $pass . ' | sudo -S -u ' . $user . ' bash script.sh');
...
/*ELSE AGENT ONLY*/
else if(isset($_POST['Portainer']) && ($_POST['Portainer']=="PAGENT"))
{	
	echo 'Creating a new Portainer endpoint. <br>';

					/*---------------------------------------------------PORTAINER---AGENT-------------------------------------------------*/
	
	if(shell_exec('echo ' . $pass . ' | sudo -S -u ' . $user . ' docker service ls | grep portainer/agent')=="")
	{	
		if($_POST['AG'])
		{	
			$LID=$_POST['AG'];	
 ...
 \" > portagent.yml");
	...
	else
	{
		...
		else 
		{
			...
			$adv= trim(shell_exec("echo azerty123 | sudo -S -u lqss sudo -S docker node inspect self --format '{{ .Status.Addr  }}' &"));
			if (shell_exec(" echo " . $pass . " | sudo -S -u " . $user . " sshpass -p " . $pass . " ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no " . $user . "@localhost \"echo " . $pass . " | sudo -S curl -kL -X POST " . $_POST['machip'] . ":9000/api/endpoints -H 'Authorization: Bearer ".$result2."' -H 'accept: application/json'  --form Name=" . $_POST['EN'] . " --form EndpointType=2 --form URL=tcp://".$adv.":9001 --form TLS=true --form TLSSkipVerify=true --form TLSSkipClientVerify=true 2>&1\" | grep Containers")!="")
			{
...
{	
	...
	$cl='echo ' . $pass . ' | sudo -S -u ' . $user . ' git clone https://github.com/stefanprodan/swarmprom.git';
	addFunction($cl);
	...
	if(isset($_POST['SA'], $_POST['SP']))
	{
		...
		{	
			$cl='echo ' . $pass . ' | sudo -S -u ' . $user . ' ADMIN_USER=' . $_POST["SA"] . ' ADMIN_PASSWORD=' . $_POST["SP"] . ' SLACK_URL='.$_POST["SU"].' SLACK_CHANNEL='.$_POST["SC"].' SLACK_USER='.$_POST["SM"].' docker stack deploy -c docker-compose.yml mon';
		}
		else
		{
			$cl='echo ' . $pass . ' | sudo -S -u ' . $user . ' ADMIN_USER=' . $_POST["SA"] . ' ADMIN_PASSWORD=' . $_POST["SP"] . ' SLACK_URL=https://hooks.slack.com/services/TOKEN SLACK_CHANNEL=devops-alerts SLACK_USER=alertmanager docker stack deploy -c docker-compose.yml mon';
		}	
	}
	else
	{
		if($_POST['SU'] && $_POST['SM'] && $_POST['SC'])
		{	
			$cl='echo ' . $pass . ' | sudo -S -u ' . $user . ' ADMIN_USER=admin ADMIN_PASSWORD=admin SLACK_URL='.$_POST["SU"].' SLACK_CHANNEL='.$_POST["SC"].' SLACK_USER='.$_POST["SM"].' docker stack deploy -c docker-compose.yml mon';
		}
		else
		{
			$cl='echo ' . $pass . ' | sudo -S -u ' . $user . ' ADMIN_USER=admin ADMIN_PASSWORD=admin SLACK_URL=https://hooks.slack.com/services/TOKEN SLACK_CHANNEL=devops-alerts SLACK_USER=alertmanager docker stack deploy -c docker-compose.yml mon';
		}	
	}
	addFunction($cl);
